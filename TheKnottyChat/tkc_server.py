...
class Server(threading.Thread):
    ...
    def run(self):  # Runs server code automatically upon new instance
        ...
        print(f"{current_time()} [SERVER]: Server is listening on {self.host} at port {self.port}")
    ...
    def receive_message(client_socket):
        ...
        try:
            ...
            message_data = cipher.decrypt(client_socket.recv(message_length))
...
class Client(threading.Thread):
    ...
    def login_handler(self):
        while True:
            ...
            if username_data['msg_data'].lower() == 'q':
    ...
    def list_accounts(self):
        ...
        account_table.field_names = ["ID", "Username", "Password", "Status"]
        account_list = [accounts for accounts in db.session.query(db.Table).all()]
        for account in account_list:
            status = 'Online' if account.status else 'Offline'
            account_table.add_row([account.userid, account.username, account.password, status])
    ...
    def delete_account(self):
        while True:
            ...
            if del_username in username_list:
                server.send_message(self.client_socket, '200')
                message_packet = server.receive_message(self.client_socket)
                print(f"{current_time()} [{self.client_socket.getpeername()}]"
                      f"<ADMIN>@AccountDeletionPage> {message_packet['msg_data']}")

                if message_packet['msg_data'] == 'y':
                    ...
                    # Any other inputs will return to top
                    continue
            ...
            try:
                ...
                if message_packet:
                    ...
                    if choice == '1':
                        ...
                        if auto_chat:
                            auto_chat = False
                            server.send_message(self.client_socket, "\n[SERVER]: Automatic chatting disabled!")
                            print(f"{current_time()} [SERVER]: Automatic chatting disabled!")

                        else:
                            auto_chat = True
                            server.send_message(self.client_socket, "\n[SERVER]: Automatic chatting enabled!")
